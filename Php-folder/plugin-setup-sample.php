<?php
/*
    Plugin Name: SBX Genesis Collection
    Plugin URI: sbx-genesis-collection
    Description: The SBX Collection for Genesis Blocks
    Version: 0.1.3
    Author: Springbox
    Author URI: https://www.springbox.com
    License: GPLv3
...
define("SBX_GENESIS_PLUGIN_URL", plugin_dir_url(__FILE__));
define("SBX_GENESIS_PLUGIN_DIR", plugin_dir_path(__FILE__));
 ...
 */
if (!function_exists('register_sbx_genesis_nav')) {
    function register_sbx_genesis_nav()
    {
        register_nav_menu('top_nav', __('SBX Top Menu'));
        register_nav_menu('main_nav', __('SBX Primary Menu'));
        register_nav_menu('footer_nav', __('SBX Footer Menu'));
    }

    add_action('init', 'register_sbx_genesis_nav');
}
 ...
 * @package SBX Genesis Collection
 */
if (!function_exists('sbx_genesis_search_form')) {
    function sbx_genesis_search_form($form)
    {
    ...
    add_filter('get_search_form', 'sbx_genesis_search_form');
}

// Add PHP Template Method blocks from within the plugin
function add_sbx_genesis_manual_blocks()
{
...
add_action('genesis_custom_blocks_add_blocks', 'add_sbx_genesis_manual_blocks');

// Add/include Collection from collection-hooks/layouts folder here
function sbx_genesis_collection()
{
    ...
    function sbx_genesis_collections_plugin_script()
    {
        // Add plugin specific js
        wp_register_script('sbx-genesis-slick-js', plugins_url('/js/slick.min.js', __FILE__), array('jquery'));
        wp_enqueue_script('sbx-genesis-slick-js');
        wp_register_script('sbx-genesis-slick-lightbox-js', plugins_url('/js/slick-lightbox.min.js', __FILE__), array('jquery'));
        wp_enqueue_script('sbx-genesis-slick-lightbox-js');
        wp_register_script('sbx-genesis-collections-js', plugins_url('/js/custom.js', __FILE__), array('jquery'));
        wp_register_script('sbx-genesis-resource-articles-js', plugins_url('/js/resource-article.js', __FILE__), array('jquery'));
        wp_enqueue_script('sbx-genesis-resource-articles-js');
        wp_enqueue_script('sbx-genesis-collections-js');
        wp_localize_script('sbx-genesis-collections-js', 'admin', array(
                'ajaxurl' => admin_url('admin-ajax.php'))
        );
        // Add plugin specific js

        wp_localize_script('sbx-genesis-collections-post-filter', 'sbx-genesis_ajax', array(
                'ajax_url' => admin_url('admin-ajax.php'))
        );

        // Add plugin specific css
        wp_enqueue_style('sbx-genesis-collections-css-material-icon', 'https://fonts.googleapis.com/css2?family=Material+Icons');
        wp_enqueue_style('sbx-genesis-collections-css-variables', plugins_url('/css/variables.css', __FILE__));
        wp_enqueue_style('sbx-genesis-collections-styles', plugins_url('/css/custom.css', __FILE__));
    }

    add_action('wp_enqueue_scripts', 'sbx_genesis_collections_plugin_script');
    add_action('admin_enqueue_scripts', 'sbx_genesis_collections_plugin_script', 0);

    // Remove other collections
    function non_brand_collection_remover()
    {
        // Clean up other collections and only show the sbx genesis collection and sections
        add_filter(
            ...
            function ($layouts) {
                ...
                return [
                    ...
                    'sbx-genesis-paginated-post-list',
...
add_action('plugins_loaded', 'sbx_genesis_collection', 12);
...
$mo_file_path = sprintf(
    '%s/%s-%s.mo',
    SBX_GENESIS_PLUGIN_DIR . 'languages',
    'sbx-genesis-collections',
    $locale
);
$domain_path = path_join(SBX_GENESIS_PLUGIN_DIR, 'languages');
